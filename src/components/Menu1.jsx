...
export default function Menu1({dropdownId, isMobile}) {
    ...
    return (
        ...
        <li>
            <button id={"dropdownSportButton"+dropdownId} data-dropdown-toggle={"dropdownSport"+dropdownId} className={`${isMobile ? 'hover:bg-light hover:text-primary w-full px-4 py-2 rounded-xl' : ''} inline-flex items-center`} type="button">
                Deportes 
            ...
            <div id={"dropdownSport"+dropdownId} className="z-10 hidden bg-primary divide-y divide-light rounded-xl shadow w-52 lg:w-72 xl:w-96">
                <ul className="py-2 text-lg lg:text-xl xl:text-2xl text-white" aria-labelledby={"dropdownSportButton"+dropdownId}>
                    <li>
